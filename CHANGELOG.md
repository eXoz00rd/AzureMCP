...
### Changed
- **Tools return less when no limit is given, so one call no longer fills a small model's context window.** `list_branches`, `list_pull_requests`, `list_my_pull_requests`, `list_release_approvals`, `list_work_item_comments`, and `get_work_item_revisions` now return at most 25 items instead of 100 (`top`), and `get_build_log` and `get_file_content` return at most 8 000 characters instead of 30 000 (`maxChars`). Defaults that were already smaller are unchanged: `list_builds`, `list_releases`, and `list_commits` still return 20 items and `list_repository_items` still returns 500. The maximums are unchanged, so a caller that needs more still asks for it with `top` up to 1 000 or `maxChars` up to 1 000 000, and every truncated result still reports that it was cut. Callers that relied on the old defaults must now pass the limit explicitly ([#67](https://github.com/eXoz00rd/AzureMCP/issues/67))
- **Wiki read tools are bounded and report truncation.** A single call could return a whole wiki tree or a whole page, enough to overflow a small model's context window. `list_wiki_pages` takes `top` (25 by default), keeps pages level by level so every top-level section shows before any deep one, and reports `totalPages` and `truncated`. `get_wiki_page` takes `maxChars` (8 000 by default) and a `startLine`/`endLine` range and reports `totalChars`, `totalLines`, and `truncated`, and no longer carries `subPages` ([#69](https://github.com/eXoz00rd/AzureMCP/issues/69))
- **Pull request read tools are bounded and report truncation.** A single call could return every changed file or every comment thread of a pull request. `get_pull_request_changes` and `list_pull_request_threads` take `top` and return at most 25 items by default, and `list_pull_request_threads` can leave out the vote and policy threads the server writes itself with `excludeSystemThreads` ([#69](https://github.com/eXoz00rd/AzureMCP/issues/69))
- **Pull request results changed shape.** `get_pull_request_changes` and `list_pull_request_threads` return `{ "items": [...], "truncated": false }` instead of a bare array, and comments now include their `commentType`
- **The server refuses an HTTP token shorter than 32 characters.** A guessable `ADOS_HTTP_TOKEN`, such as the word `token`, let anyone who could reach the endpoint call it. A deployment with a shorter token now stops at startup and says how to generate one, for example with `openssl rand -hex 32` ([#65](https://github.com/eXoz00rd/AzureMCP/pull/65))
- **Refusing to start is a clean exit.** A setting the server cannot run with ends it with exit code 1 and the reason as the last line, instead of an unhandled exception that an orchestrator reports as a crash ([#65](https://github.com/eXoz00rd/AzureMCP/pull/65))
- **An `ADOS_HTTP_URL` the server cannot listen on is refused at startup.** It must be a plain `http://` address with a host and a numeric port; before, a port Kestrel could not read made it listen on port 80 without a word, and other malformed addresses crashed it ([#65](https://github.com/eXoz00rd/AzureMCP/pull/65))

### Fixed
- **Two failures that surfaced only as "An error occurred invoking …" now explain themselves:** an Azure DevOps Server that never answers, because a firewall drops the connection, and a collection URL answered by a web page instead of the API, such as a site or a sign-in page. A change whose request timed out warns that it may still have been applied ([#65](https://github.com/eXoz00rd/AzureMCP/pull/65))
