...
public static class Toolsets
{
    ...
    private static readonly Dictionary<string, Type> Registry = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["projects"] = typeof(ProjectTools),
        ["workitems"] = typeof(WorkItemTools),
        ["queries"] = typeof(QueryTools),
        ["repositories"] = typeof(RepositoryTools),
        ["pullrequests"] = typeof(PullRequestTools),
        ["builds"] = typeof(BuildTools),
        ["releases"] = typeof(ReleaseTools),
        ["wiki"] = typeof(WikiTools)
    };
    ...
    public static IReadOnlyList<Type> Resolve(string? requested)
    {
        ...
        if (unknown.Count > 0)
        {
            throw new InvalidOperationException(
                $"{AzureDevOpsServerOptions.ToolsetsVariable} contains unknown toolsets: {string.Join(", ", unknown)}. " +
