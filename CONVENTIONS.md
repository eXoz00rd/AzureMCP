...
No real or test Azure DevOps Server instance is currently available in this environment. Until one is, a task may satisfy its real-server verification requirement by extending `StubHttpMessageHandler` / `StubAzureDevOpsServer` to model the behavior in question and verifying against that stub instead — state this substitution explicitly in the Definition of Done rather than silently dropping the requirement. This substitution does not apply to a task whose purpose is discovering undocumented real-server behavior (a capability spike): a stub can only replay behavior we already encode into it, so it cannot answer a question we do not yet know the answer to. Such tasks stay blocked in the backlog until a real or test server is available.
